...
func TestRPN_NRPN_JustLSBCallback(t *testing.T) {
	tests := []struct {
		write       func(w *writer.Writer)
		description string
		expected    string
	}{
		// RPN
		{
			func(w *writer.Writer) { writer.RPN(w, 0, 0, 23, 0) },
			"simple RPN without LSB",
			"RPN(0,0) LSB on channel 0: 0 | ",
		},
		{
			func(w *writer.Writer) { w.SetChannel(1); writer.RPN(w, 0, 0, 23, 0) },
			"simple RPN without LSB channel 1",
			"RPN(0,0) LSB on channel 1: 0 | ",
		},
		{
			func(w *writer.Writer) { writer.RPN(w, 1, 1, 43, 54) },
			"simple RPN with LSB",
		...
		// NRPN
		{
			func(w *writer.Writer) { writer.NRPN(w, 0, 0, 23, 0) },
			"simple NRPN without LSB",
			"NRPN(0,0) LSB on channel 0: 0 | ",
		},
		{
			func(w *writer.Writer) { w.SetChannel(1); writer.NRPN(w, 0, 0, 23, 0) },
			"simple NRPN without LSB channel 1",
			"NRPN(0,0) LSB on channel 1: 0 | ",
		},
		{
			func(w *writer.Writer) { writer.NRPN(w, 1, 1, 43, 54) },
	...
	for _, test := range tests {
		...
		rd.message.Channel.ControlChange.Each = func(p *Position, ch, cc, val uint8) {
			fmt.Fprintf(&out, "CC%v on channel %v: %v | ", cc, ch, val)
		}

		rd.message.Channel.ControlChange.RPN.LSB = func(p *Position, ch, ident1, ident2, lsb uint8) {
			fmt.Fprintf(&out, "RPN(%v,%v) LSB on channel %v: %v | ", ident1, ident2, ch, lsb)
		}

		rd.message.Channel.ControlChange.NRPN.LSB = func(p *Position, ch, ident1, ident2, lsb uint8) {
...
func TestRPN_NRPN(t *testing.T) {
	...
	for _, test := range tests {
		...
		rd.message.Channel.ControlChange.RPN.MSB = func(p *Position, ch, ident1, ident2, msb uint8) {
		...
		rd.message.Channel.ControlChange.RPN.Increment = func(p *Position, ch, ident1, ident2 uint8) {
			fmt.Fprintf(&out, "RPN(%v,%v) Increment on channel %v | ", ident1, ident2, ch)
		}

		rd.message.Channel.ControlChange.RPN.Decrement = func(p *Position, ch, ident1, ident2 uint8) {
			fmt.Fprintf(&out, "RPN(%v,%v) Decrement on channel %v | ", ident1, ident2, ch)
		}

		rd.message.Channel.ControlChange.RPN.Reset = func(p *Position, ch uint8) {
			fmt.Fprintf(&out, "RPN RESET on channel %v | ", ch)
		}

		rd.message.Channel.ControlChange.NRPN.MSB = func(p *Position, ch, ident1, ident2, msb uint8) {
			fmt.Fprintf(&out, "NRPN(%v,%v) on channel %v MSB %v | ", ident1, ident2, ch, msb)
		}

		rd.message.Channel.ControlChange.NRPN.Reset = func(p *Position, ch uint8) {
		...
		rd.message.Channel.ControlChange.NRPN.Increment = func(p *Position, ch, ident1, ident2 uint8) {
			fmt.Fprintf(&out, "NRPN(%v,%v) Increment on channel %v | ", ident1, ident2, ch)
		}

		rd.message.Channel.ControlChange.NRPN.Decrement = func(p *Position, ch, ident1, ident2 uint8) {
...
func TestTimeAt(t *testing.T) {
	...
	for _, test := range tests {
		...
		wr := writer.NewSMF(&bf, 1, smfwriter.TimeFormat(mt))
		writer.TempoBPM(wr, test.tempo1)
		wr.SetDelta(twobars)
		writer.TempoBPM(wr, test.tempo2)
		wr.SetDelta(twobars)
		wr.SetDelta(twobars * 8)
		writer.NoteOn(wr, 64, 120)
		wr.SetDelta(twobars)
